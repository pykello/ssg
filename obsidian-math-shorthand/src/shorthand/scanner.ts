import type { MathRange } from "./types";
...
export function scanMathRanges(source: string): MathRange[] {
  const ranges: MathRange[]=[]; let i=0; let fence: string|undefined;
  while(i<source.length){
    const lineStart=i===0||source[i-1]==="\n";
    if(lineStart){ const lineEnd=source.indexOf("\n",i); const line=source.slice(i,lineEnd<0?source.length:lineEnd); const match=/^\s*(```|~~~)/.exec(line); if(match){fence=fence?undefined:match[1]; i=lineEnd<0?source.length:lineEnd+1; continue;} }
